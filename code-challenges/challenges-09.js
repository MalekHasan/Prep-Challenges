...
const employeesBonus = (arr) => {
  ...
  arr.forEach((item) => {
    if (item.workHours > 8) {
      item.salary =
        Number(item.salary.slice(0, item.salary.lastIndexOf("$"))) + 100 + "$";
      result.push(item);
    } else if (item.workHours <= 8) {
      item.salary =
        Number(item.salary.slice(0, item.salary.lastIndexOf("$"))) + 50 + "$";
      result.push(item);
    }
  });
  return result;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
  ...
  mostExpensive(budget, mouseArray, keyBoardArray);
