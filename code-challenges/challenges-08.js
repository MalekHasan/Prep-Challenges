...
const customerAndAge = (obj) => {
  ...
  for (const property in obj) {
    let cName=`Customer Name :${property} , Age :${obj[property]}`;
    custArr.push(cName);
  }
  return custArr;
};
...
const getEntries = (obj) => {
  ...
  let entArr=[];
  for (const property in obj) {
    let entObj=`${property}: ${obj[property]}`;
    entArr.push(entObj);
  }
  return entArr;
};
...
//   coursesName.push(iterator.course);
//   for (let index = 0; index < iterator.Students.length; index++) {
    
//   }
//   let obj={
//     Students: iterator.Students,
//     course : iterator.course,
//   }
//   studentsName.push(iterator.Students);
