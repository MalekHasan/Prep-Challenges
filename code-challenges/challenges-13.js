...
const objectCounter = (obj) => {
    ...
    const counter=obj.reduce((acc,element,i)=>{
        acc++;
    return acc;    
    },0)
    return counter;
...
const stringReverse = (str) => {
    // write your code here
    const reverseString=str.split(' ').reduce((acc,element,i)=>{
        acc.unshift(element);
        return acc;
    },[])
...
const statistics = (obj) => {
    // write your code here
    const numbers =obj.reduce((acc,item,i)=>{
        if(acc[item.votes_To]== undefined){
            acc[item.votes_To]=0;
            acc[item.votes_To]++;
        }
        else {
