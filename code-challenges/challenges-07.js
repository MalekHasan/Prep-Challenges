...
const applicationsStatics = (arr) => {
    ...
    if (arr[i].firstName && arr[i].lastName || arr[i].firstName||arr[i].lastName) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].tech=="JS") {
...
const classesAvg = (data) => {
    ...
    {
        let classNum = gradesArr[i].classes;
        for (let j = 0; j < classNum.length; j++)
        {
            ...
            data.grades[i].classes[j].avg = Math.floor(sum / scores.length);
        sum=0;
        }
    }
    return data;
